...
def plot_player_season_trend(entity_data, entity_name, entity_type="Gracz", min_monthly_attempts=10):
    ...
    monthly_eff['Miesiąc'] = monthly_eff['GAME_DATE'].dt.strftime('%Y-%m') # Format YYYY-MM dla osi X
...
if load_status.get("success", False) and not shooting_data.empty:
    ...
    if st.session_state.active_view == "📊 Rankingi Skuteczności":
        # === Zawartość Widoku 1: Rankingi Skuteczności (Przearanżowane) ===

        # Użyjemy typu sezonu w głównym nagłówku
        ...
        with c1_dist:
            if 'SHOT_TYPE' in filtered_data.columns and not filtered_data['SHOT_TYPE'].isnull().all():
                # Usunięcie NaN przed liczeniem, jeśli istnieją
                ...
                if not shot_type_counts.empty:
                     ...
                     fig_type.update_layout(legend_title_text='Typ Rzutu') # Dodano tytuł legendy
        ...
        with c2_dist:
            if 'ACTION_TYPE' in filtered_data.columns and not filtered_data['ACTION_TYPE'].isnull().all():
                 # Usunięcie NaN przed liczeniem
                action_type_counts = filtered_data['ACTION_TYPE'].dropna().value_counts().head(15).reset_index()
                if not action_type_counts.empty:
                    fig_action = px.bar(action_type_counts, y='ACTION_TYPE', x='count', orientation='h', title='Najczęstsze Typy Akcji (Top 15)', labels={'count':'Liczba Rzutów', 'ACTION_TYPE':''})
                    fig_action.update_layout(yaxis={'categoryorder':'total ascending'}, height=400) # Zmniejszono wysokość
        ...
        num_bins_freq = st.slider("Dokładność mapy (liczba kwadratów na oś X):", 20, 80, 50, 5, key='frequency_map_bins_slider_reorder') # Zmieniono klucz dla pewności
        # Użyj proporcji boiska NBA (szerokość 50ft, długość połowy ~47ft -> ~94ft całk.) -> Y bins ~= X bins * 1.9
        ...
        # Użyj unikalnych kluczy, jeśli te same były używane gdzie indziej
        ...
        # Obliczenia rankingów (bez zmian)
        ...
        st.markdown("###### Skuteczność Ogółem (FG%)") # Zmieniono na h6 dla mniejszego odstępu
        ...
        # (Kod z poprzedniej odpowiedzi - bez zmian)
...
# ... (kod dla poprzednich elifów bez zmian) ...
        ...
        * **Cel:** Zaprezentowanie **szczegółowych metryk** (Raport Klasyfikacji) i **wizualizacji błędów** (Macierz Pomyłek) dla *jednego konkretnego* podziału danych (którego proporcje można dostosować). Pomaga to zrozumieć, *jakie* błędy model popełnia.
        * **Jak działa:** Dane są jednorazowo dzielone na zbiór treningowy i testowy zgodnie z wybranym procentem. Ten sam `Pipeline` jest trenowany na zbiorze treningowym i oceniany na zbiorze testowym.
        * **Wyniki:**
        ...
        **Podsumowanie:** Walidacja krzyżowa daje lepszy obraz *ogólnej* wydajności, podczas gdy pojedynczy podział dostarcza *szczegółowego wglądu* w działanie modelu na jednym przykładzie. Wyniki z pojedynczego podziału mogą być mniej stabilne i zależeć od losowego podziału danych (`random_state=42` zapewnia powtarzalność tego konkretnego podziału dla tych samych danych wejściowych i proporcji).
            ...
            if not player_model_data.empty:
                ...
                if all(feat in player_model_data.columns for feat in model_features) and target_variable in player_model_data.columns:
                    ...
                    if pmdc[target_variable].nunique() != 2 and not pmdc.empty:
                         st.warning(f"Po usunięciu brakujących wartości w '{target_variable}', pozostała tylko jedna klasa wyniku rzutu ({int(pmdc[target_variable].unique()[0]) if not pmdc.empty else 'brak'}). Nie można zbudować modelu.")
                    elif pmdc.empty:
                         st.warning(f"Brak ważnych danych dla gracza '{selected_player}' po przetworzeniu.")
                    else:
                        ...
                        if len(pmdc) >= min_samples_for_model:
                            ...
                            if st.button(f"Uruchom Ocenę Modelu KNN dla {selected_player}", key='run_eval_knn_button'):
                                ...
                                st.markdown("---")
                                st.subheader(f"2. Szczegółowa Ocena na Podziale ({train_size_percent}%/{test_size_percent}%)")
                                ...
                                with st.spinner(f"Przygotowanie i ocena na podziale {train_size_percent}/{test_size_percent} (k={k})..."):
                                    try:
                                        ...
                                        if len(X_test) == 0 or len(y_test) == 0:
                                            ...
                                            # (Wyświetlanie raportu i macierzy pomyłek bez zmian)
                        ...
                        # Komunikaty o błędach ilości danych
                             ...
                             st.warning(f"Niewystarczająca ilość danych ({len(pmdc)} próbek po usunięciu NaN) dla gracza '{selected_player}' do zbudowania wiarygodnego modelu. Wymagane minimum: {min_samples_for_model}.")
                ...
                # Komunikat o błędzie brakujących kolumn
                    ...
                    st.warning(f"Brak wymaganych kolumn dla '{selected_player}' w danych: {', '.join(missing_cols)}")
