...
# --- Funkcje Pomocnicze ---
# (Funkcje load_shooting_data, add_court_shapes, filter_data_by_player,
...
#  calculate_top_performers - BEZ ZMIAN, wklej je tutaj z poprzedniej wersji)
# --- POCZĄTEK FUNKCJI POMOCNICZYCH (skopiuj z poprzedniej wersji) ---
@st.cache_data
...
def calculate_hot_zones(entity_data, min_shots_in_zone=5, n_bins=10):
    ...
    x_min, x_max = -250, 250
    y_min, y_max = -47.5, 422.5 # Użyj granic boiska z add_court_shapes
...
def calculate_top_performers(data, group_by_col, min_total_shots, min_2pt_shots, min_3pt_shots, top_n=10):
    ...
    if 'SHOT_TYPE' in valid_data.columns:
         ...
         # Sprawdź, czy wartość istnieje w danych, zanim spróbujesz filtrować
         if shot_type_2pt in valid_data['SHOT_TYPE'].unique():
             data_2pt = valid_data[valid_data['SHOT_TYPE'] == shot_type_2pt]
             if not data_2pt.empty: # Dodano sprawdzenie, czy filtrowanie dało wyniki
    ...
    if 'SHOT_TYPE' in valid_data.columns:
       # SHOT_TYPE już powinien być stringiem z poprzedniego kroku
        if shot_type_3pt in valid_data['SHOT_TYPE'].unique():
            ...
            if not data_3pt.empty: # Dodano sprawdzenie
...
if load_status.get("success", False) and not shooting_data.empty:
    ...
    selected_player = st.sidebar.selectbox(
        "Gracz do analizy/modelowania:", options=available_players, index=default_player_index if default_player_index is not None else 0,
        key='player_select', disabled=not available_players,
        help="Wybierz gracza do szczegółowej analizy lub oceny modeli predykcyjnych."
    )
    ...
    selected_team = st.sidebar.selectbox(
        "Drużyna do analizy:", options=available_teams, index=default_team_index if default_team_index is not None else 0,
    ...
    tab_options = [
        ...
        "🏀 Analiza Zespołowa",
        "🎯 Model Predykcji (KNN)",
        "🎯 Model Predykcji (XGBoost)",
        "📊 Porównanie Modeli (KNN vs XGBoost)" # <--- NOWA ZAKŁADKA PORÓWNANIA
    ]

    # Użyj st.radio do wyboru widoku, ustawiając domyślną wartość z session_state
    # Sprawdź, czy bieżący widok jest nadal dostępny, jeśli nie, wróć do domyślnego
    ...
    if st.session_state.active_view == "📊 Rankingi Skuteczności":
        # (Kod bez zmian - skopiuj z poprzedniej wersji)
        ...
        # Oblicz nbins_y proporcjonalnie do zakresu osi Y/X ~ (480 - (-60))/(260 - (-260)) = 540/520 ~ 1.04
        # Użyjemy stałej proporcji dla uproszczenia, np. 1.0
        ...
        tt_ov, tt_2, tt_3 = calculate_top_performers(filtered_data, 'TEAM_NAME', min_total*5, min_2pt*5, min_3pt*5) # Zwiększono min. dla zespołów
        ...
        **Podsumowanie:** Walidacja krzyżowa daje lepszy obraz *ogólnej* wydajności rozszerzonego modelu, podczas gdy pojedynczy podział dostarcza *szczegółowego wglądu* w jego działanie. Wyniki z pojedynczego podziału mogą zależeć od losowego podziału (`random_state=42`). Dodanie cech kategorycznych może (ale nie musi) poprawić dokładność modelu, ale zwiększa też złożoność (więcej wymiarów po OHE).
        ...
        # (Kod bez zmian - skopiuj z poprzedniej wersji)
        # === Zawartość Zakładki 6: Model Predykcji (XGBoost) ===
    ...
    # ==========================================================================
    # === NOWA ZAKŁADKA: Porównanie Modeli ======================================
    # ==========================================================================
        ...
        st.markdown(f"""
        Ta zakładka pozwala na bezpośrednie porównanie wyników modeli **KNN** i **XGBoost** wytrenowanych na danych dla gracza **{selected_player}**.
        Używane są te same cechy wejściowe i ten sam potok przetwarzania danych (One-Hot Encoding dla cech kategorycznych, StandardScaler dla numerycznych).

        Poniżej możesz skonfigurować parametry oceny (wspólne dla obu modeli w tym porównaniu) i uruchomić obliczenia, aby zobaczyć wyniki obok siebie.
        """)
        ...
        if selected_player:
            ...
            if not player_model_data.empty:
                # Definicja cech (identyczna jak w KNN/XGBoost)
                numerical_features = ['LOC_X', 'LOC_Y', 'SHOT_DISTANCE']
                categorical_features = ['ACTION_TYPE', 'SHOT_TYPE', 'PERIOD']
                target_variable = 'SHOT_MADE_FLAG'
                all_features = numerical_features + categorical_features
                ...
                if all(feat in player_model_data.columns for feat in all_features) and target_variable in player_model_data.columns:
                    ...
                    # Przygotowanie danych (identyczne jak w KNN/XGBoost)
                    pmdc = player_model_data[all_features + [target_variable]].dropna().copy()
                    pmdc[target_variable] = pd.to_numeric(pmdc[target_variable], errors='coerce')
                    pmdc = pmdc.dropna(subset=[target_variable])
                    for col in categorical_features:
                    ...
                    # Sprawdzenie klas i ilości danych
                    if pmdc[target_variable].nunique() != 2 and not pmdc.empty:
                        st.warning(f"Po usunięciu brakujących wartości w '{target_variable}' lub cechach, pozostała tylko jedna klasa wyniku rzutu ({int(pmdc[target_variable].unique()[0]) if not pmdc.empty else 'brak'}). Nie można porównać modeli.")
                    elif pmdc.empty:
                        st.warning(f"Brak ważnych danych (po usunięciu NaN we wszystkich cechach: {', '.join(all_features)}, {target_variable}) dla gracza '{selected_player}'.")
                    else:
                        pmdc[target_variable] = pmdc[target_variable].astype(int)
                        ...
                        if len(pmdc) >= min_samples_for_model:
                            ...
                            # Używamy UNIKALNYCH kluczy dla tej zakładki
                            k_comp = st.slider(
                                "Liczba sąsiadów (k dla KNN):", min_value=3, max_value=min(25, len(pmdc)//3) if len(pmdc) >= 9 else 3,
                                value=5, step=2, key='comp_knn_k'
                            )
                            n_splits_comp = st.slider(
                                "Liczba podziałów walidacji krzyżowej (folds):", min_value=3, max_value=10, value=5, step=1,
                                key='comp_cv_splits'
                            )
                            test_size_percent_comp = st.slider(
                                "Rozmiar zbioru testowego (%):",
                                min_value=10, max_value=50,
                                value=20, step=5,
                                key='comp_test_split',
                            ...
                            if st.button(f"Uruchom Porównanie KNN vs XGBoost dla {selected_player}", key='run_comparison_button'):

                                # --- Przygotowanie danych (raz) ---
                                X = pmdc[all_features]
                                y = pmdc[target_variable]
                                X_train, X_test, y_train, y_test = train_test_split(
                                    X, y, test_size=test_size_float_comp, random_state=42, stratify=y
                                )

                                if len(X_test) == 0 or len(y_test) == 0:
                                     st.error(f"Wybrany rozmiar zbioru testowego ({test_size_percent_comp}%) skutkuje pustym zbiorem testowym dla dostępnych danych ({len(y)} próbek). Nie można przeprowadzić porównania. Wybierz większy rozmiar.")
                                else:
                                    # --- Definicja preprocesora (raz) ---
                                    preprocessor = ColumnTransformer(
                                        transformers=[
                                            ('cat', OneHotEncoder(handle_unknown='ignore', sparse_output=False), categorical_features),
                                            ('num', StandardScaler(), numerical_features)
                                    ...
                                    pipeline_knn = Pipeline([
                                        ('preprocessor', preprocessor),
                                        ('knn', KNeighborsClassifier(n_neighbors=k_comp))
                                    ])
                                    ...
                                    # --- Wyniki do przechowywania ---
                                    results = {"knn": {}, "xgb": {}}

                                    # --- 1. Walidacja Krzyżowa (Oba Modele) ---
                                    st.markdown("---")
                                    st.subheader(f"Porównanie Walidacji Krzyżowej ({n_splits_comp} folds)")
                                    col_cv1, col_cv2 = st.columns(2)

                                    with col_cv1:
                                        st.markdown("**KNN**")
                                        with st.spinner(f"CV dla KNN (k={k_comp}, folds={n_splits_comp})..."):
                                            try:
                                                ...
                                                results["knn"]["cv_mean"] = scores_knn.mean()
                                                results["knn"]["cv_std"] = scores_knn.std()
                                                st.metric("Średnia Dokładność:", f"{results['knn']['cv_mean']:.2%}")
                                                st.metric("Odch. Std.:", f"{results['knn']['cv_std']:.4f}")
                                                st.caption("Dokładności foldów: " + ", ".join([f"{s:.2%}" for s in scores_knn]))
                                            except Exception as e:
                                                results["knn"]["cv_mean"] = "Błąd"
                                                st.error(f"Błąd CV KNN: {e}")

                                    with col_cv2:
                                        st.markdown("**XGBoost**")
                                        with st.spinner(f"CV dla XGBoost (folds={n_splits_comp})..."):
                                            try:
                                                cv_xgb = StratifiedKFold(n_splits=n_splits_comp, shuffle=True, random_state=42)
                                                scores_xgb = cross_val_score(pipeline_xgb, X, y, cv=cv_xgb, scoring='accuracy', n_jobs=-1)
                                                results["xgb"]["cv_mean"] = scores_xgb.mean()
                                                results["xgb"]["cv_std"] = scores_xgb.std()
                                                st.metric("Średnia Dokładność:", f"{results['xgb']['cv_mean']:.2%}")
                                                st.metric("Odch. Std.:", f"{results['xgb']['cv_std']:.4f}")
                                                st.caption("Dokładności foldów: " + ", ".join([f"{s:.2%}" for s in scores_xgb]))
                                            except Exception as e:
                                                results["xgb"]["cv_mean"] = "Błąd"
                                                st.error(f"Błąd CV XGBoost: {e}")

                                    # --- 2. Ocena na Pojedynczym Podziale (Oba Modele) ---
                                    st.markdown("---")
                                    st.subheader(f"Porównanie na Podziale Testowym ({test_size_percent_comp}%)")
                                    col_s1, col_s2 = st.columns(2)

                                    with col_s1:
                                        st.markdown("**KNN**")
                                        with st.spinner(f"Ocena KNN na podziale (k={k_comp})..."):
                                            try:
                                                ...
                                                st.metric(f"Dokładność:", f"{results['knn']['single_acc']:.2%}")

                                                # Macierz Pomyłek KNN
                                                fig_cm_knn = px.imshow(results["knn"]["conf_matrix"], text_auto=True, aspect="auto",
                                                                     labels=dict(x="Predykcja", y="Prawda", color="Liczba"),
                                                                     x=['Niecelny', 'Celny'], y=['Niecelny', 'Celny'],
                                                                     color_continuous_scale=px.colors.sequential.Blues,
                                                                     title=f"Macierz Pomyłek KNN (k={k_comp})")
                                                fig_cm_knn.update_layout(coloraxis_showscale=False, height=300, title_font_size=14, margin=dict(t=40, b=10))
                                                st.plotly_chart(fig_cm_knn, use_container_width=True)
                                                cm_knn = results["knn"]["conf_matrix"]
                                                st.caption(f"TN={cm_knn[0,0]}, FP={cm_knn[0,1]}, FN={cm_knn[1,0]}, TP={cm_knn[1,1]}")

                                            except Exception as e:
                                                results["knn"]["single_acc"] = "Błąd"
                                                st.error(f"Błąd oceny KNN: {e}")

                                    with col_s2:
                                        st.markdown("**XGBoost**")
                                        with st.spinner("Ocena XGBoost na podziale..."):
                                            try:
                                                pipeline_xgb.fit(X_train, y_train)
                                                y_pred_xgb = pipeline_xgb.predict(X_test)
                                                results["xgb"]["single_acc"] = accuracy_score(y_test, y_pred_xgb)
                                                results["xgb"]["conf_matrix"] = confusion_matrix(y_test, y_pred_xgb)
                                                st.metric(f"Dokładność:", f"{results['xgb']['single_acc']:.2%}")

                                                # Macierz Pomyłek XGBoost
                                                fig_cm_xgb = px.imshow(results["xgb"]["conf_matrix"], text_auto=True, aspect="auto",
                                                                     labels=dict(x="Predykcja", y="Prawda", color="Liczba"),
                                                                     x=['Niecelny', 'Celny'], y=['Niecelny', 'Celny'],
                                                                     color_continuous_scale=px.colors.sequential.Greens,
                                                                     title="Macierz Pomyłek XGBoost")
                                                fig_cm_xgb.update_layout(coloraxis_showscale=False, height=300, title_font_size=14, margin=dict(t=40, b=10))
                                                st.plotly_chart(fig_cm_xgb, use_container_width=True)
                                                cm_xgb = results["xgb"]["conf_matrix"]
                                                st.caption(f"TN={cm_xgb[0,0]}, FP={cm_xgb[0,1]}, FN={cm_xgb[1,0]}, TP={cm_xgb[1,1]}")

                                            except Exception as e:
                                                results["xgb"]["single_acc"] = "Błąd"
                                                st.error(f"Błąd oceny XGBoost: {e}")

                                    st.caption(f"Porównanie na {len(y_test)} próbkach testowych ({test_size_percent_comp}%). Zbiór treningowy: {len(y_train)} próbek.")


                        else: # len(pmdc) < min_samples_for_model
                            st.warning(f"Niewystarczająca ilość danych ({len(pmdc)} próbek po usunięciu NaN) dla gracza '{selected_player}' do porównania modeli. Wymagane minimum: {min_samples_for_model}.")
                else:
                    missing_cols = [col for col in all_features + [target_variable] if col not in player_model_data.columns]
                    st.warning(f"Brak wymaganych kolumn dla '{selected_player}' w danych: {', '.join(missing_cols)}. Potrzebne: {', '.join(all_features + [target_variable])}")
            else:
            ...
            st.info("Wybierz gracza z panelu bocznego, aby porównać modele.")
        # === Koniec Zakładki 7: Porównanie Modeli ===
    # ==========================================================================
