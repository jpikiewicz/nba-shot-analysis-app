    ...
    if st.session_state.active_view == "📊 Rankingi Skuteczności":
        ...
        with c1_dist: # Rozkład typów rzutów
            if 'SHOT_TYPE' in filtered_data.columns and not filtered_data['SHOT_TYPE'].isnull().all():
                # ... (kod wykresu kołowego typów rzutów) ...
        ...
        with c2_dist: # Typy akcji (częstotliwość i skuteczność)
            if 'ACTION_TYPE' in filtered_data.columns and not filtered_data['ACTION_TYPE'].isnull().all():
                # ... (kod wykresu słupkowego najczęstszych akcji) ...
            ...
            else: st.caption("Brak kolumny 'ACTION_TYPE'.")
            ...
            if 'ACTION_TYPE' in filtered_data.columns and 'SHOT_MADE_FLAG' in filtered_data.columns:
                min_attempts_eff_action = st.number_input(
                    "Min. prób dla rankingu skuteczności akcji:", min_value=5, value=10, step=1,
                    key='rank_min_attempts_eff_action',
                    help="Minimalna liczba rzutów danego typu akcji w rankingu."
                )
                # ... (kod obliczania i rysowania najefektywniejszych akcji) ...
                ...
                if not action_eff_data.empty:
                    ...
                    if not action_stats_filtered.empty:
                        ...
                        fig_action_eff = px.bar(
                            action_stats_filtered, y='ACTION_TYPE', x='FG%', orientation='h',
                            title=f'Najefektywniejsze Typy Akcji (Top 15, min. {min_attempts_eff_action} prób)',
                            labels={'FG%': 'Skuteczność (%)', 'ACTION_TYPE': ''}, text='FG%', hover_data=['count']
                        )
                    ...
                    else: st.caption(f"Brak typów akcji z min. {min_attempts_eff_action} prób.")
                else: st.caption("Brak danych do obliczenia skuteczności akcji.")
            else: st.caption("Brak kolumn 'ACTION_TYPE' lub 'SHOT_MADE_FLAG'.")

        st.markdown("---")
        st.subheader(f"Mapa Częstotliwości Rzutów")
        # ... (kod mapy częstotliwości z kluczem 'rank_frequency_map_bins') ...
        ...
        # ... (kod rankingów z kluczami 'rank_min_total', 'rank_min_2pt', 'rank_min_3pt') ...
        ...
        # ... (kod wyświetlania rankingów 2PT FG%) ...
        ...
        # ... (kod wyświetlania rankingów 3PT FG%) ...
        ...
        if selected_player and 'PLAYER_NAME' in filtered_data.columns:
            ...
            if not player_data.empty:
                # ... (kod sekcji statystyk gracza) ...
                st.subheader("Statystyki Podstawowe")
                # ... (kod obliczania i wyświetlania metryk) ...
                ...
                # ... (kod wykresu sk. vs odl. z kluczami 'player_eff_dist_bin', 'player_eff_dist_min') ...
                ...
                # ... (kod hot zones z kluczami 'player_hotzone_min_shots', 'player_hotzone_bins') ...
                ...
                # ... (kod analizy czasowej z kluczami 'player_quarter_min_shots', 'player_month_min_shots') ...
                ...
                # ... (kod analizy grupowej z kluczem 'player_group_min_shots') ...
        ...
        if len(selected_players_compare) >= 2:
            ...
            # ... (kod filtrowania danych dla porównania) ...
            ...
            if not compare_data_filtered.empty:
                st.subheader("Skuteczność vs Odległość")
                # ... (kod porównania sk. vs odl. z kluczami 'comp_eff_dist_bin', 'comp_eff_dist_min') ...
                ...
                # ... (kod porównania wg strefy z kluczem 'comp_zone_min') ...
                ...
                # ... (kod wyświetlania map rzutów dla porównywanych graczy z dynamicznymi kluczami) ...
        ...
        if selected_team and 'TEAM_NAME' in filtered_data.columns:
            ...
            if not team_data.empty:
                # ... (kod analizy zespołowej analogiczny do analizy gracza, z kluczami team_*) ...
        ...
        # ... (kod opisu i logiki oceny KNN z kluczami knn_eval_*) ...
        st.markdown(f"""### Interpretacja Wyników Modelu KNN ({selected_player}) ... """) # Pełny opis jak poprzednio
        if selected_player:
            ...
            if not player_model_data.empty:
                ...
                if all(feat in player_model_data.columns for feat in all_features) and target_variable in player_model_data.columns:
                    ...
                    if pmdc[target_variable].nunique() != 2 and not pmdc.empty:
                         ...
                         st.warning(f"Brak ważnych danych dla gracza '{selected_player}'.")
                    else:
                        ...
                        if len(pmdc) >= min_samples_for_model:
                            ...
                            if st.button(f"Uruchom Ocenę Modelu KNN dla {selected_player}", key='knn_eval_run_button'):
                                preprocessor = ColumnTransformer(
                                    transformers=[('cat', OneHotEncoder(handle_unknown='ignore', sparse_output=False, drop='first'), categorical_features),
                                ...
                                # ... (kod oceny na pojedynczym podziale dla KNN) ...
                                with st.spinner(f"Ocena KNN na podziale (k={k})..."):
                                     try:
                                         ...
                                         if len(X_test) == 0: st.warning("Pusty zbiór testowy.")
                                             ...
                                             # ... (wyświetlanie CM) ...
                                             fig_cm = px.imshow(cm, text_auto=True, labels=dict(x="Predykcja", y="Prawda"), x=['Niecelny', 'Celny'], y=['Niecelny', 'Celny'], title="Macierz Pomyłek KNN")
                                             st.plotly_chart(fig_cm, use_container_width=True)
                                     except Exception as e: st.error(f"Błąd oceny KNN: {e}")
                        else: st.warning(f"Niewystarczająca ilość danych ({len(pmdc)}) dla KNN. Minimum: {min_samples_for_model}.")
        ...
        # ... (kod opisu i logiki oceny XGBoost z kluczami xgb_eval_* i sekcją SHAP) ...
        st.markdown(f"""### Interpretacja Wyników Modelu XGBoost ({selected_player}) ... """) # Pełny opis jak poprzednio
        if selected_player:
            player_model_data = filter_data_by_player(selected_player, filtered_data)
            if not player_model_data.empty:
                numerical_features = ['LOC_X', 'LOC_Y', 'SHOT_DISTANCE']
                    ...
                    if pmdc[target_variable].nunique() != 2 and not pmdc.empty:
                        ...
                        if len(pmdc) >= min_samples_for_model:
                            ...
                            if st.button(f"Uruchom Ocenę Modelu XGBoost dla {selected_player}", key='xgb_eval_run_button'):
                                ...
                                # ... (kod oceny na pojedynczym podziale dla XGBoost) ...
                                with st.spinner(f"Ocena XGBoost na podziale..."):
                                     try:
                                         ...
                                         if len(X_test) == 0: st.warning("Pusty zbiór testowy.")
                                             ...
                                             # ... (wyświetlanie CM) ...
                                             fig_cm_xgb = px.imshow(cm_xgb, text_auto=True, labels=dict(x="Predykcja", y="Prawda"), x=['Niecelny', 'Celny'], y=['Niecelny', 'Celny'], title="Macierz Pomyłek XGBoost", color_continuous_scale=px.colors.sequential.Greens)
                                             ...
                                             with st.spinner("Obliczanie wartości SHAP..."):
                                                 try:
                                                     ...
                                                     st.markdown("Pozycja X: Wpływ na predykcję 'Celny'. Kolor: Wartość cechy (Czerwony=Wysoka).")
                                                     ...
                                                     plt.clf()
                                                 except Exception as e_shap: st.error(f"Błąd SHAP: {e_shap}")
                                             # === KONIEC SEKCJI SHAP ===

                                     except Exception as e: st.error(f"Błąd oceny XGBoost: {e}")
                        else: st.warning(f"Niewystarczająca ilość danych ({len(pmdc)}) dla XGBoost. Minimum: {min_samples_for_model}.")
                else: st.warning(f"Brak wymaganych kolumn dla '{selected_player}'.")
            else: st.warning(f"Brak danych dla gracza '{selected_player}'.")
        ...
        # ... (kod porównania modeli z kluczami model_comp_*) ...
        st.markdown(f"""Porównanie wyników KNN i XGBoost dla gracza **{selected_player}**. """) # Pełny opis jak poprzednio
        if selected_player:
             player_model_data = filter_data_by_player(selected_player, filtered_data)
             if not player_model_data.empty:
                numerical_features = ['LOC_X', 'LOC_Y', 'SHOT_DISTANCE']
                    ...
                    if pmdc[target_variable].nunique() != 2 and not pmdc.empty:
                        ...
                        if len(pmdc) >= min_samples_for_model:
                            ...
                            if st.button(f"Uruchom Porównanie KNN vs XGBoost dla {selected_player}", key='model_comp_run_button'):
                                ...
                                if len(X_test) == 0: st.error("Pusty zbiór testowy. Nie można porównać.")
                                else:
                                    preprocessor = ColumnTransformer(
                                        transformers=[('cat', OneHotEncoder(handle_unknown='ignore', sparse_output=False, drop='first'), categorical_features),
                                    ...
                                    # ... (kod CV dla KNN i XGBoost w kolumnach) ...
                                    with col_cv1:
                                         st.markdown("**KNN**")
                                    ...
                                    # ... (kod oceny na podziale testowym dla KNN i XGBoost w kolumnach) ...
                                    with col_s1:
                                         st.markdown("**KNN**")
                                         with st.spinner("Ocena KNN..."):
                                             try:
                                                 ...
                                                 fig_cm_knn = px.imshow(results["knn"]["conf_matrix"], text_auto=True, x=['N', 'C'], y=['N', 'C'], title=f"CM KNN (k={k_comp})")
                                    ...
                                    with col_s2:
                                         ...
                                         with st.spinner("Ocena XGBoost..."):
                                             try:
                                                 ...
                                                 fig_cm_xgb = px.imshow(results["xgb"]["conf_matrix"], text_auto=True, x=['N', 'C'], y=['N', 'C'], title="CM XGBoost", color_continuous_scale=px.colors.sequential.Greens)
                        ...
                        else: st.warning(f"Niewystarczająca ilość danych ({len(pmdc)}) do porównania. Minimum: {min_samples_for_model}.")
                else: st.warning(f"Brak wymaganych kolumn dla '{selected_player}'.")
             else: st.warning(f"Brak danych dla gracza '{selected_player}'.")
