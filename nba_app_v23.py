# nba_app_v6_tabs_no_extra_text.py
# Revision History:
# v1-v4: Previous versions (Base, KNN, XGBoost, SHAP plots, UI tweaks)
# v5: Added dynamic SHAP interpretation comment
# v6: Replaced st.radio for view selection with st.tabs, removed extra text from success msg.
...
# === Usunięto inicjalizację Session State dla active_view - st.tabs zarządza tym wewnętrznie ===
...
if load_status.get("success", False) and not shooting_data.empty:
    ...
    # USUNIĘTO: Fragment o dodaniu ACTION_TYPE_SIMPLE
    ...
    # === ZMIANA: Użycie st.tabs zamiast st.radio ===
    tab_options = [
        ...
        "📊 Porównanie Modeli" # Skrócono nazwę dla lepszego wyglądu zakładek
    ]
    ...
    # --- Zawartość zakładki Analiza Gracza ---
    with tab_player:
    ...
    # --- Zawartość zakładki Porównanie Graczy ---
    with tab_compare:
    ...
    # --- Zawartość zakładki Analiza Zespołowa ---
    with tab_team:
    ...
    # --- Zawartość zakładki Ocena Modelu KNN ---
    with tab_knn:
        # Prefiks dla kluczy widgetów: knn_eval_
    ...
    # --- Zawartość zakładki Ocena Modelu XGBoost ---
    with tab_xgb:
                                                 ...
                                                 try:
                                                     ...
                                                     try:
                                                         ...
                                                         if not feature_importance_df.empty:
                                                             ...
                                                             try:
                                                                 # Znajdź indeks cechy w oryginalnej liście
                                                                 ...
                                                                 if not np.isnan(correlation):
                                                                     ...
                                                                     comment += impact_description
                                                                      ...
                                                                      comment += " Nie udało się jednoznacznie określić ogólnego kierunku wpływu tej cechy."
                                                             except IndexError:
                                                                  comment += f" Nie udało się znaleźć indeksu dla cechy '{top_feature_name}'."
                                                             except Exception as e_corr:
                                                                 comment += f" Nie udało się określić kierunku wpływu (błąd: {e_corr})."

                                                             st.caption(comment) # Wyświetl podsumowanie
                                                         else:
    ...
    # --- Zawartość zakładki Porównanie Modeli ---
    with tab_model_comp:
        # Prefiks dla kluczy widgetów: model_comp_
        ...
        Używane są te same dane wejściowe i ten sam potok przetwarzania wstępnego. Porównywane są wyniki z walidacji krzyżowej i pojedynczego podziału testowego.
        """)
                            ...
                            if st.button(f"Uruchom Porównanie KNN vs XGBoost dla {selected_player}", key='model_comp_run_button'):
                                ...
                                if X_test.empty:
                                    ...
                                    if isinstance(results['knn'].get('cv_mean'), float) and isinstance(results['xgb'].get('cv_mean'), float):
                                        ...
                                        else: winner_cv = "Remis" # Explicitly check for tie
                                    ...
                                    if isinstance(results['knn'].get('single_acc'), float) and isinstance(results['xgb'].get('single_acc'), float):
                                         ...
                                         else: winner_test = "Remis" # Explicitly check for tie
