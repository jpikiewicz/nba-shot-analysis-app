# nba_app_v7_shap_sampling.py
# Revision History:
# v1-v4: Previous versions (Base, KNN, XGBoost, SHAP plots, UI tweaks)
# v5: Added dynamic SHAP interpretation comment
# v6: Replaced st.radio with st.tabs, removed extra success msg text
# v7: Implemented SHAP data sampling for performance optimization in XGBoost tab
...
# --- Funkcje Pomocnicze ---
# (Wszystkie funkcje pomocnicze pozostają takie same jak w poprzedniej wersji)
...
if load_status.get("success", False) and not shooting_data.empty:
    ...
    # Komunikat o sukcesie (bez info o ACTION_TYPE_SIMPLE)
    ...
    with tab_rankings:
        # Cały kod dla zakładki Rankingi (bez zmian)
        st.header(f"Analiza Ogólna Sezonu: {selected_season_type}")
        st.markdown("---")
        # ... (reszta kodu dla tej zakładki jak w poprzedniej wersji) ...
    ...
    with tab_player:
        # Cały kod dla zakładki Analiza Gracza (bez zmian)
    ...
    with tab_compare:
        # Cały kod dla zakładki Porównanie Graczy (bez zmian)
    ...
    with tab_team:
        # Cały kod dla zakładki Analiza Zespołowa (bez zmian)
    ...
    with tab_knn:
        # Cały kod dla zakładki Ocena KNN (bez zmian)
        ...
        * Algorytm KNN działa na podstawie odległości między punktami danych w przestrzeni cech. Aby uwzględnić cechy nienumeryczne, zostały one przekształcone za pomocą **One-Hot Encoding (OHE)**.
        * Cechy numeryczne (`LOC_X`, `LOC_Y`, `SHOT_DISTANCE`) są **skalowane** za pomocą `StandardScaler`.
        * OHE i skalowanie są wykonywane w ramach `Pipeline` za pomocą `ColumnTransformer`.
        ...
        if selected_player:
            ...
            if not player_model_data.empty:
                ...
                if 'ACTION_TYPE_SIMPLE' in player_model_data.columns:
                     # Checkbox do wyboru kolumny akcji
                     ...
                     st.caption(f"Model KNN użyje kolumny: `{action_type_col_model}`") # Informacja dla użytkownika
    ...
    with tab_xgb:
        ...
        # --- ZMIANA: Dodano komentarz o próbkowaniu SHAP ---
        st.markdown(f"""
        ### Interpretacja Wyników Modelu XGBoost ({selected_player})

        Ta zakładka prezentuje model **XGBoost (Extreme Gradient Boosting)**. Używa tych samych cech i przetwarzania co KNN (chyba, że inaczej wskazano poniżej).
        ...
        **3. Interpretacja SHAP:** Pokazuje wpływ poszczególnych cech na predykcje modelu dla danych testowych. Zawiera automatyczne podsumowanie najważniejszej cechy.
           * **Uwaga:** W celu optymalizacji wydajności (szczególnie w środowiskach z ograniczonymi zasobami, jak Streamlit Cloud), analiza SHAP może być przeprowadzana na losowej próbce danych testowych (domyślnie do 500 punktów).
        """)
        # --- KONIEC ZMIANY ---
                                     ...
                                     try:
                                         ...
                                         if X_test.empty:
                                             ...
                                             with st.spinner("Obliczanie wartości SHAP..."):
                                                 try:
                                                     # Pobierz wytrenowany model i preprocesor
                                                     ...
                                                     feature_names_out = [str(fn) for fn in feature_names_out]

                                                     # --- ZMIANA: Użycie próbki danych dla SHAP ---
                                                     n_shap_samples = 500 # Max liczba próbek do analizy SHAP
                                                     if len(X_test_transformed) > n_shap_samples:
                                                         st.info(f"Ze względu na wydajność, analiza SHAP zostanie przeprowadzona na losowej próbce {n_shap_samples} punktów z danych testowych ({len(X_test_transformed)} dostępnych).")
                                                         # Użyj prostego indeksowania numpy do wylosowania próbek
                                                         sample_indices = np.random.choice(X_test_transformed.shape[0], n_shap_samples, replace=False)
                                                         X_test_shap_sample = X_test_transformed[sample_indices]
                                                     else:
                                                         # Jeśli danych testowych jest mniej niż próbka, użyj wszystkich
                                                         X_test_shap_sample = X_test_transformed
                                                         n_shap_samples = len(X_test_shap_sample) # Aktualizuj liczbę próbek
                                                         st.info(f"Analiza SHAP zostanie przeprowadzona na wszystkich {n_shap_samples} punktach danych testowych.")

                                                     # Stwórz DataFrame z próbki danych (potrzebne do summary_plot)
                                                     X_test_shap_sample_df = pd.DataFrame(X_test_shap_sample, columns=feature_names_out)
                                                     # --- Koniec ZMIANY ---

                                                     # Oblicz wartości SHAP tylko dla próbki
                                                     shap_values_sample = explainer.shap_values(X_test_shap_sample) # Użyj X_test_shap_sample

                                                     # Wybierz wartości SHAP dla klasy pozytywnej (dla próbki)
                                                     if isinstance(shap_values_sample, list) and len(shap_values_sample) == 2:
                                                         shap_values_for_plot = shap_values_sample[1]
                                                     else:
                                                         shap_values_for_plot = shap_values_sample

                                                     # Wykres Beeswarm (na próbce)
                                                     st.markdown("#### Globalna Ważność Cech (Summary Plot - Beeswarm)")
                                                     st.markdown("Pozycja X: Wpływ na predykcję 'Celny'. Kolor: Wartość cechy.")
                                                     fig_summary, ax_summary = plt.subplots()
                                                     # Użyj X_test_shap_sample_df i shap_values_for_plot
                                                     shap.summary_plot(shap_values_for_plot, X_test_shap_sample_df, feature_names=feature_names_out, show=False, plot_type="beeswarm")
                                                     st.pyplot(fig_summary, bbox_inches='tight')
                                                     plt.clf()

                                                     # Wykres Bar (na próbce)
                                                     st.markdown("#### Średnia Absolutna Wartość SHAP (Ważność Cech - Bar)")
                                                     fig_bar, ax_bar = plt.subplots()
                                                     # Użyj X_test_shap_sample_df i shap_values_for_plot
                                                     shap.summary_plot(shap_values_for_plot, X_test_shap_sample_df, feature_names=feature_names_out, show=False, plot_type="bar")
                                                     st.pyplot(fig_bar, bbox_inches='tight')
                                                     plt.clf()

                                                     # Sekcja interpretacji tekstowej (też bazuje na próbce)
                                                     try:
                                                         st.markdown("---")
                                                         st.markdown("##### Podsumowanie Interpretacji SHAP (na podstawie próbki)")
                                                         ...
                                                         if not feature_importance_df.empty:
                                                             ...
                                                             comment = f"Dla gracza **{selected_player}**, najważniejszą cechą (na podst. próbki {n_shap_samples} pkt.) była **`{top_feature_name}`** (średni abs. wpływ SHAP: {top_feature_importance:.3f})."
                                                             try:
                                                                 top_feature_original_index = list(feature_names_out).index(top_feature_name)
                                                                 top_feature_values = X_test_shap_sample_df[top_feature_name].values # Użyj wartości z próbki
                                                                 top_feature_shap_values = shap_values_for_plot[:, top_feature_original_index] # Te już są dla próbki
                                                                 ...
                                                                 if not np.isnan(correlation):
                                                                     ...
                                                                     if correlation < -0.1:
                                                                         if is_numerical_heuristic: impact_description = f" Generalnie, **wyższe wartości** tej cechy **zmniejszają** przewidywaną szansę na trafienie."
                                                                         else: impact_description = f" Obecność tej cechy (wartość 1) generalnie **zmniejsza** przewidywaną szansę na trafienie."
                                                                     elif correlation > 0.1:
                                                                          if is_numerical_heuristic: impact_description = f" Generalnie, **wyższe wartości** tej cechy **zwiększają** przewidywaną szansę na trafienie."
                                                                          else: impact_description = f" Obecność tej cechy (wartość 1) generalnie **zwiększa** przewidywaną szansę na trafienie."
                                                                     else: impact_description = " Kierunek jej wpływu wydaje się być zróżnicowany lub słaby."
                                                                     comment += impact_description
                                                                 else: comment += " Nie udało się określić ogólnego kierunku wpływu."
                                                             except IndexError: comment += f" Błąd indeksowania dla cechy '{top_feature_name}'."
                                                             except Exception as e_corr: comment += f" Błąd analizy korelacji: {e_corr}."
                                                             st.caption(comment)
                                                         else: st.caption("Nie znaleziono ważnych cech w analizie SHAP.")
                                                     except Exception as e_interp: st.warning(f"Błąd generowania interpretacji SHAP: {e_interp}")

                                                 except ImportError: st.error("Biblioteka SHAP lub Matplotlib nie jest zainstalowana.")
                                                 except Exception as e_shap: st.error(f"Błąd obliczania lub wizualizacji SHAP: {e_shap}")
                                             # === KONIEC SEKCJI SHAP ===
    ...
    with tab_model_comp:
        # Cały kod dla zakładki Porównanie Modeli (bez zmian)
...
# Obsługa przypadku, gdy dane nie zostały wczytane poprawnie
    ...
    # Sprawdź, czy jest zapisana wiadomość o błędzie w stanie sesji
