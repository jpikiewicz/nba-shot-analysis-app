...
# v7: Implemented SHAP data sampling for performance optimization
...
if load_status.get("success", False) and not shooting_data.empty:
    ...
    tab_options = [
        ...
        "🏀 Analiza Zespołowa",
        "🎯 Ocena Modelu (KNN)",
        "🎯 Ocena Modelu (XGBoost)", # Nazwa zakładki pozostaje, ale bez SHAP
        "📊 Porównanie Modeli"
    ]
    ...
    # ... (Kod dla zakładek tab_rankings, tab_player, tab_compare, tab_team, tab_knn - bez zmian w stosunku do v7) ...
    # Poniżej wklejam tylko kod dla zakładek Ocena XGBoost i Porównanie Modeli dla zwięzłości
    # (Kod dla pozostałych zakładek jest taki sam jak w poprzedniej odpowiedzi)

    with tab_rankings:
        # Cały kod dla zakładki Rankingi (bez zmian)
        st.header(f"Analiza Ogólna Sezonu: {selected_season_type}")
        st.markdown("---")
        # ... (reszta kodu dla tej zakładki jak w poprzedniej wersji v7) ...
                                ...
                                with st.spinner(f"Trenowanie i ocena XGBoost na pojedynczym podziale..."):
                                     try:
                                         ...
                                         if X_test.empty: st.warning("Zbiór testowy jest pusty po podziale.")
                                             ...
                                             # --- ZMIANA: Cała sekcja SHAP została usunięta ---
