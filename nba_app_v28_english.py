...
# v10: Added detailed descriptions to KNN and XGBoost model evaluation tabs
...
# --- Helper Functions ---
# (All helper functions remain the same as in version v9)
...
def plot_player_season_trend(entity_data, entity_name, entity_type="Player", min_monthly_attempts=10):
    ...
    # Use 'M' instead of 'ME' for compatibility with newer pandas and to avoid warnings
...
st.title("🏀 Interactive Analysis of NBA Player Shots (Season 2023-24)")
...
if load_status.get("success", False) and not shooting_data.empty:
    ...
    if load_status.get("missing_key_cols"): st.warning(f"Missing key columns for full analysis: {', '.join(load_status['missing_key_cols'])}.")
    ...
    with tab_rankings:
        # All code for the Rankings tab (no changes from v9)
        ...
        with c1_top:
            ...
            if 'SHOT_TYPE' in filtered_data.columns and not filtered_data['SHOT_TYPE'].isnull().all():
                ...
                if not shot_type_counts.empty:
                    # Rename column from 'index' to 'SHOT_TYPE' if pandas named it so
                    ...
                    # Rename column from 'SHOT_TYPE' to 'count' if pandas named it so
        ...
        with c2_top:
            ...
            if action_type_col_rank in filtered_data.columns and not filtered_data[action_type_col_rank].isnull().all():
                ...
                # Similar check for column names as above
        ...
        with c1_bottom:
            ...
            if 'SHOT_ZONE_BASIC' in filtered_data.columns and not filtered_data['SHOT_ZONE_BASIC'].isnull().all():
                ...
                # Similar check for column names
        ...
        st.markdown("###### 2-Point Effectiveness (2PT FG%)")
        ...
        st.markdown("###### 3-Point Effectiveness (3PT FG%)")
    ...
    with tab_player:
        # All code for the Player Analysis tab (no changes from v9)
    ...
    with tab_compare:
        # All code for the Player Comparison tab (no changes from v9)
    ...
    with tab_team:
        # All code for the Team Analysis tab (no changes from v9)
    ...
    with tab_knn:
        ...
        # --- START CHANGE: New, detailed description for KNN ---
        st.markdown(f"""
        ...
        **Feature Importance:**
        ...
        if selected_player:
             ...
             if not player_model_data.empty:
                  ...
                  categorical_features = [action_type_col_model, 'SHOT_TYPE', 'PERIOD'] # Using the variable defined before description
    ...
    with tab_xgb:
        ...
        # --- START CHANGE: New, detailed description for XGBoost ---
        st.markdown(f"""
        ...
        **Data Preparation (Preprocessing):**
        ...
        2.  **Encoding Categorical Features:** "One-Hot Encoding" (`OneHotEncoder` with `drop='first'` and `handle_unknown='ignore'`) for `{action_type_col_model_xgb}`, `SHOT_TYPE`, `PERIOD`. This is necessary for the model to process these features.
        ...
        **Feature Importance (Interpretability):**
        ...
        if selected_player:
            ...
            if not player_model_data.empty:
                ...
                categorical_features = [action_type_col_model_xgb, 'SHOT_TYPE', 'PERIOD'] # Using the variable defined before description
                target_variable = 'SHOT_MADE_FLAG'
                all_features = numerical_features + categorical_features
    ...
    with tab_model_comp:
        # All code for the Model Comparison tab (no changes from v9)
        ...
        The same input data and the same preprocessing pipeline (scaling numerical features, One-Hot Encoding categorical features) are used for both models to ensure a fair comparison.
...
# --- Sidebar Footer ---
st.sidebar.markdown("---")
# st.sidebar.info("Extended Streamlit Application - NBA Analysis v10") # Updated version
try:
    ...
    # Use the current date (2025-04-18)
    # ts = datetime.now(tz).strftime('%Y-%m-%d %H:%M:%S %Z')
    # Correction: use the date provided in the context
    current_dt_context = datetime(2025, 4, 18, 19, 5, 33) # Time from context (assuming it's local time in Wroclaw)
    ...
    # Fallback if pytz is not installed, use system time
    # import sys # Needed to check in 'sys.modules'
    # current_dt_context = datetime(2025, 4, 18, 19, 5, 33)
    # ts = current_dt_context.strftime('%Y-%m-%d %H:%M:%S (local time - no pytz)')
    print("Warning: Library 'pytz' is not installed.")
    # Safer fallback without importing sys in the except block
    ts = datetime(2025, 4, 18, 19, 5, 33).strftime('%Y-%m-%d %H:%M:%S (local time - no pytz)')


except Exception as e_time:
    # Even more general fallback
    # ts = datetime.now().strftime('%Y-%m-%d %H:%M:%S (local time - timezone error)')
    ts = datetime(2025, 4, 18, 19, 5, 33).strftime('%Y-%m-%d %H:%M:%S (local time - timezone error)')
