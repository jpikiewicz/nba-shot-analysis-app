...
# - Zmodyfikowaną zakładkę Analiza Gracza (ikona, statystyki na górze)
...
def get_basic_stats(entity_data, entity_name, entity_type="Gracz"):
    ...
    if 'SHOT_MADE_FLAG' in entity_data.columns and stats['total_shots'] > 0:
        # Używamy tylko poprawnych numerycznych wartości flagi
        ...
        if not numeric_shots.empty:
            ...
            stats['made_shots'] = 0 # Jeśli brak poprawnych danych, ustaw na 0
...
def calculate_hot_zones(entity_data, min_shots_in_zone=5):
    ...
    x_bins = np.linspace(-300, 300, 11) # 10 stref wzdłuż osi X
    y_bins = np.linspace(-50, 450, 11)  # 10 stref wzdłuż osi Y
    ...
    zones = zones.dropna(subset=['x_center', 'y_center']) # Usuń jeśli środek nie mógł być obliczony
    return zones
...
def plot_hot_zones_heatmap(hot_zones_df, entity_name, entity_type="Gracz"):
    ...
    # Sprawdzenie i konwersja typów
    for col in required_cols:
        if not pd.api.types.is_numeric_dtype(hot_zones_df[col]):
            try:
                 hot_zones_df[col] = pd.to_numeric(hot_zones_df[col])
            except ValueError:
                 st.warning(f"Problem z konwersją kolumny '{col}' na typ numeryczny w danych hot zone.")
                 return None
    ...
    # Ustawienie zakresu kolorów
    color_range = [max(0, min_pct - 5), min(100, max_pct + 5)] # Dajmy trochę marginesu
    ...
    fig = px.scatter(hot_zones_df, x='x_center', y='y_center',
                     ...
                     custom_data=['made_shots', 'total_shots'] # Dodaj trafione i próby do hovera
                    )
...
def plot_player_quarter_eff(entity_data, entity_name, entity_type="Gracz"):
    ...
    # Upewnij się, że PERIOD jest int
    try:
         quarter_data['PERIOD'] = quarter_data['PERIOD'].astype(int)
    except ValueError:
         st.warning("Problem z konwersją kolumny 'PERIOD' na int dla analizy kwart.")
         return None


    quarter_eff = quarter_data.groupby('PERIOD')['SHOT_MADE_FLAG'].agg(['mean', 'count']).reset_index()
    quarter_eff['mean'] *= 100
    quarter_eff = quarter_eff[quarter_eff['count'] >= 5] # Min 5 rzutów w kwarcie
...
def plot_player_season_trend(entity_data, entity_name, entity_type="Gracz"):
    ...
    # Upewnij się, że GAME_DATE jest datą
    ...
    if trend_data.empty or len(trend_data) < 10: return None # Min 10 rzutów ogółem
    trend_data = trend_data.set_index('GAME_DATE')

    # Resample miesięcznie ('ME' - Month End)
    monthly_eff = trend_data.resample('ME')['SHOT_MADE_FLAG'].agg(['mean', 'count']).reset_index()
    monthly_eff['mean'] *= 100
    monthly_eff = monthly_eff[monthly_eff['count'] >= 10] # Min 10 rzutów w miesiącu
    if monthly_eff.empty or len(monthly_eff) < 2: return None # Min 2 miesiące z danymi
    ...
    if not pd.isna(monthly_eff['mean'].max()):
         max_y = min(100, monthly_eff['mean'].max() * 1.1 + 5) # Dynamiczna oś Y
...
def plot_grouped_effectiveness(entity_data, group_col, entity_name, entity_type="Gracz", top_n=10):
    ...
    grouped_eff = grouped_eff[grouped_eff['count'] >= 5] # Min 5 rzutów w grupie
    grouped_eff = grouped_eff.sort_values(by='count', ascending=False).head(top_n) # Sortuj wg liczby, pokaż top N
    ...
    fig.update_traces(texttemplate='%{text:.1f}%', textposition='outside')
    fig.update_layout(yaxis_range=[0, 100], uniformtext_minsize=8, uniformtext_mode='hide',
                      xaxis_title=axis_label, xaxis={'categoryorder':'total descending'}) # Sortuj kategorie na osi X wg liczby prób
    return fig
...
def plot_comparison_eff_distance(compare_data, selected_players):
    ...
    bin_width = 3 # Szerokość przedziału odległości (w stopach)
    ...
    min_shots_per_bin = 5 # Min rzutów w przedziale odległości
...
# --- NOWA FUNKCJA DO PORÓWNANIA WG STREF ---
@st.cache_data
def plot_comparison_eff_by_zone(compare_data, selected_players, min_shots_per_zone=5):
    ...
    Args:
        compare_data (pd.DataFrame): DataFrame zawierający dane rzutów dla wybranych graczy.
        selected_players (list): Lista nazw wybranych graczy.
        min_shots_per_zone (int): Minimalna liczba rzutów w danej strefie dla danego gracza,
                                   aby uwzględnić go w porównaniu dla tej strefy.
    ...
    # Sprawdzenie, czy niezbędne kolumny istnieją
    ...
    # Upewnij się, że SHOT_MADE_FLAG jest numeryczna i usuń NaN w kluczowych kolumnach
    ...
    # Oblicz statystyki (trafione, próby) dla każdego gracza w każdej strefie
    zone_stats = zone_eff_data.groupby(['PLAYER_NAME', 'SHOT_ZONE_BASIC'], observed=False)['SHOT_MADE_FLAG'].agg(
        Made='sum',
        Attempts='count'
    ).reset_index()

    # Odfiltruj kombinacje gracz/strefa ze zbyt małą liczbą prób
    zone_stats_filtered = zone_stats[zone_stats['Attempts'] >= min_shots_per_zone]

    if zone_stats_filtered.empty:
         st.info(f"Brak wystarczających danych (min. {min_shots_per_zone} prób na strefę przez gracza) dla wybranych graczy do porównania skuteczności wg stref.")
         return None

    # Oblicz FG%
    zone_stats_filtered['FG%'] = (zone_stats_filtered['Made'] / zone_stats_filtered['Attempts']) * 100

    # --- Definiowanie logicznej kolejności stref na osi X ---
    # !!! WAŻNE: Dostosuj tę listę do RZECZYWISTYCH nazw stref w Twoim pliku CSV !!!
    # Sprawdź unikalne wartości np. przez: st.write(compare_data['SHOT_ZONE_BASIC'].unique())
    zone_order = [
        'Restricted Area',
        'In The Paint (Non-RA)',
        'Mid-Range',
        ...
        'Right Corner 3',
        'Above the Break 3',
        'Backcourt' # Zazwyczaj bardzo mało rzutów, można rozważyć usunięcie
    ]
    # Ogranicz dane tylko do stref zdefiniowanych w zone_order
    ...
    # Stworzenie grupowanego wykresu słupkowego
    fig = px.bar(zone_stats_plot,
                 x='SHOT_ZONE_BASIC',
                 y='FG%',
                 color='PLAYER_NAME',   # Koloruj słupki wg gracza
                 barmode='group',      # Grupuj słupki dla każdej strefy
                 ...
                 hover_data=['Attempts', 'Made'], # Pokaż próby i trafione w podpowiedzi
                 category_orders={'SHOT_ZONE_BASIC': zone_order}, # Zastosuj kolejność stref
                 text='FG%' # Pokaż wartość % na słupkach
                )

    # Ustawienia wyglądu
    fig.update_layout(
        yaxis_range=[0, 100], # Skala Y od 0 do 100%
        xaxis={'categoryorder':'array', 'categoryarray':zone_order}, # Wymuś kolejność na osi X
        legend_title_text='Gracze' # Tytuł legendy
    )
    fig.update_traces(texttemplate='%{text:.1f}%', textposition='outside')

    return fig

# --- NOWA FUNKCJA RANKINGOWA ---
@st.cache_data
def calculate_top_performers(data, group_by_col, min_total_shots, min_2pt_shots, min_3pt_shots, top_n=10):
    ...
    if not overall_stats.empty:
        ...
        col_name = group_by_col.replace('_',' ').title() # Nazwa kolumny gracza/zespołu
    ...
    shot_type_2pt = '2PT Field Goal' # !!! DOSTOSUJ, JEŚLI TRZEBA !!!
    ...
    if 'SHOT_TYPE' in valid_data.columns:
        if shot_type_2pt in valid_data['SHOT_TYPE'].unique():
            ...
            if not stats_2pt.empty:
             ...
             st.caption(f"Nie znaleziono wartości '{shot_type_2pt}' w kolumnie SHOT_TYPE dla rankingu 2PT.")
    else:
        st.caption("Brak kolumny 'SHOT_TYPE' dla rankingu 2PT.")


    # --- Ranking za 3 punkty (3PT FG%) ---
    shot_type_3pt = '3PT Field Goal' # !!! DOSTOSUJ, JEŚLI TRZEBA !!!
    ...
    if 'SHOT_TYPE' in valid_data.columns:
        if shot_type_3pt in valid_data['SHOT_TYPE'].unique():
            ...
            if not stats_3pt.empty:
             ...
             st.caption(f"Nie znaleziono wartości '{shot_type_3pt}' w kolumnie SHOT_TYPE dla rankingu 3PT.")
    else:
         st.caption("Brak kolumny 'SHOT_TYPE' dla rankingu 3PT.")
...
if not shooting_data.empty:
    ...
    # --- Zakładki Aplikacji (z nową ikoną dla Analizy Gracza)---
    tab1, tab2, tab3, tab4, tab5 = st.tabs([
        "📊 Rankingi Skuteczności",
        "⛹️ Analiza Gracza",   # Zmieniono ikonę na koszykarza
        "🆚 Porównanie Graczy",
        "🏀 Analiza Zespołowa",
        "🎯 Model Predykcji (KNN)"
    ])
    ...
    with tab1:
        ...
        # Ustal wyższe progi dla zespołów
    ...
    # --- Zakładka 2: Analiza Gracza (ZMIENIONA STRUKTURA) ---
    with tab2:
        ...
        if selected_player:
            ...
            if not player_data.empty:
                ...
                with st.expander("Pokaż/Ukryj Rozkład Odległości i Strefy", expanded=False):
                    ...
                    if fig_p_dist: st.plotly_chart(fig_p_dist, use_container_width=True)
                    else: st.caption("Brak danych do histogramu odległości.")
    ...
    # --- Zakładka 3: Porównanie Graczy (Z DODANĄ ANALIZĄ WG STREF) ---
    with tab3:
        ...
        if len(selected_players_compare) >= 2:
            ...
            if not compare_data_filtered.empty:
                # Porównanie Skuteczność vs Odległość
                ...
                # NOWE PORÓWNANIE: Skuteczność wg Strefy Rzutowej
                st.subheader("Skuteczność wg Strefy Rzutowej (SHOT_ZONE_BASIC)")
                min_attempts_zone = st.slider(
                    "Minimalna liczba prób w strefie:", min_value=1, max_value=50, value=5, step=1,
                    key='compare_zone_min_shots_slider'
                )
                fig_comp_zone_eff = plot_comparison_eff_by_zone(
                    compare_data_filtered, selected_players_compare, min_shots_per_zone=min_attempts_zone
                )
                ...
                for i, player in enumerate(selected_players_compare):
                    with cols[i]:
                        ...
                        if not player_comp_data.empty:
                            ...
                            if fig_comp_chart:
                                fig_comp_chart.update_layout(height=450, title="") # Mniejszy wykres bez tytułu
    ...
    with tab4:
        ...
        if selected_team:
            ...
            if not team_data.empty:
                ...
                t_stats = get_basic_stats(team_data, selected_team, "Zespół") # Używamy starej funkcji dla ogólnych statystyk
                col1, col2, col3 = st.columns(3)
                ...
                # Wizualizacje dla zespołu (analogicznie do gracza)
                ...
                with st.expander("Pokaż/Ukryj Rozkład Odległości i Strefy Zespołu", expanded=False):
                    ...
                    t_hot_zones = calculate_hot_zones(team_data, min_shots_in_zone=10) # Wyższy próg dla zespołu
                ...
                fig_t_quarter = plot_player_quarter_eff(team_data, selected_team, "Zespół") # Reuse function
                ...
                # Trend sezonowy dla zespołu może być mniej sensowny, pomijamy dla uproszczenia
                # fig_t_trend = plot_player_season_trend(team_data, selected_team, "Zespół")
                        ...
                        if len(player_model_data_cleaned) > 50 and player_model_data_cleaned['SHOT_MADE_FLAG'].nunique() == 2:
                            ...
                            if st.button(f"Trenuj model KNN dla {selected_player}", key='train_model_button_v3'):
                                 with st.spinner("Trenowanie modelu... (Uwaga: Funkcjonalność niezaimplementowana)"):
                                     ...
                                     st.warning("Aby zobaczyć działanie modelu, należy dodać funkcje: `train_knn_model`, `predict_shot_probability`, `plot_probability_heatmap` oraz kod wyświetlający ich wyniki.")
                        else:
                            st.warning(f"Niewystarczająca ilość danych (wymagane > 50 rzutów z dwoma wynikami: znaleziono {len(player_model_data_cleaned)} rzutów, {player_model_data_cleaned['SHOT_MADE_FLAG'].nunique()} unikalnych wyników po czyszczeniu) dla gracza '{selected_player}' do zbudowania sensownego modelu przy obecnych filtrach.")
                    else:
                         st.warning(f"Brak poprawnych danych dla gracza '{selected_player}' do trenowania modelu po usunięciu NaN.")
                else:
                    st.warning(f"Brak wymaganych kolumn ({model_features + ['SHOT_MADE_FLAG']}) do trenowania modelu dla gracza '{selected_player}'.")
            else:
