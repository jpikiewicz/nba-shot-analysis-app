# nba_app_v2.py
# Final version incorporating all requested modifications as of 2025-04-09
# Includes default player = LeBron James, default team = Los Angeles Lakers,
# and default comparison = LeBron James & Stephen Curry
...
if not shooting_data.empty:
    ...
    # Domyślny Gracz: LeBron James
    default_player = "LeBron James"
    player_default_index = 0 # Domyślnie pierwszy gracz
    if available_players and default_player in available_players:
        try:
            player_default_index = available_players.index(default_player)
        except ValueError:
            player_default_index = 0 # Fallback, jeśli wystąpi błąd (nie powinno przy 'in' check)

    # Domyślna Drużyna: Los Angeles Lakers
    default_team = "Los Angeles Lakers"
    team_default_index = 0 # Domyślnie pierwsza drużyna
    if available_teams and default_team in available_teams:
        try:
            team_default_index = available_teams.index(default_team)
        except ValueError:
            team_default_index = 0 # Fallback

    # Domyślni Gracze do Porównania: LeBron James i Stephen Curry
    default_compare_players_names = ["LeBron James", "Stephen Curry"]
    # Wybierz tylko tych graczy, którzy faktycznie są na liście dostępnych
    default_compare_list = [p for p in default_compare_players_names if p in available_players]
    # --- Koniec ustawiania domyślnych wartości ---

    # Tworzenie widgetów z ustawionymi domyślnymi wartościami
    selected_player = st.sidebar.selectbox(
        "Gracz do analizy:",
        options=available_players,
        index=player_default_index, # Użycie obliczonego indeksu
        key='player_select',
    ...
    selected_team = st.sidebar.selectbox(
        ...
        index=team_default_index, # Użycie obliczonego indeksu
        key='team_select',
    ...
    selected_players_compare = st.sidebar.multiselect(
        ...
        options=available_players,
        default=default_compare_list, # Użycie obliczonej listy domyślnej
        max_selections=5,
